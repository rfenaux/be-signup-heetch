...
function wplra_prepare_filters_based_on_saved($filter_by,$filter_by_value,$redirect_to_url){ ?>
	...
	<div class="input-group mb-3 wplra_filtering_group_container">
	  	...
	  	<select name="wplra_select_filter_by_elem" id="wplra_select_filter_by_elem" class="form-control wplra_filter_select wplra_select_filter_by_elem">
	  		...
	  		<option value="first_name" <?php echo $filter_by == "first_name" ? 'selected' : ''; ?>>User First Name</option>
	  		<option value="last_name" <?php echo $filter_by == "last_name" ? 'selected' : ''; ?>>User Last Name</option>
		...
		<select name="wplra_filter_by_first_name" class="form-control wplra_filter_select wplra_filter_by_first_name" <?php echo $filter_by == 'first_name' ? " style='display:block;'":""; ?>>
		...
		<?php
				...
				foreach ($users as $user) {
					...
					if (!empty($user->first_name)) {
						...
						echo "<option value='$user->first_name'";

						if ($filter_by == 'first_name') {
							if ($filter_by_value == $user->first_name) {
								echo ' selected ';
							}
						}

						echo ">$user->first_name</option>";
		...
		<select name="wplra_filter_by_last_name" class="form-control wplra_filter_select wplra_filter_by_last_name" <?php echo $filter_by == 'last_name' ? " style='display:block;'":""; ?>>
		...
		<?php
				...
				foreach ($users as $user) {
					...
					if (!empty($user->last_name)) {
						...
						echo "<option value='$user->last_name'";

						if ($filter_by == 'last_name') {
							if ($filter_by_value == $user->last_name) {
								echo ' selected ';
							}
						}

						echo ">$user->last_name</option>";
